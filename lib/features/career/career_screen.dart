...
class _CareerScreenState extends ConsumerState<CareerScreen> {
  ...
  /// Displays an authentic EA Sports FC style selection alert when ineligible starters are detected.
  /// Allows manager to manually edit their squad in the Squad Hub or automatically replace with best bench options.
  Future<bool?> _showIneligibleStartersDialog(List<Player> ineligibleStarters) async {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final ink = isDark ? Colors.white : Colors.black87;
    final inkMuted = isDark ? Colors.white60 : Colors.black54;
    ...
    return showDialog<bool>(
      ...
      builder: (ctx) {
        return AlertDialog(
          backgroundColor: isDark ? AppPalette.darkSurfaceRaised : Colors.white,
          ...
          title: Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.warning_amber_rounded, color: AppPalette.coral, size: 22),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      'Squad Selection Alert',
                      style: AppTypography.titleMedium(ink).copyWith(
                    ...
                    Text(
                      '${ineligibleStarters.length} ${ineligibleStarters.length == 1 ? "starter is" : "starters are"} currently ineligible',
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'The following players in your Starting XI cannot play. You can manually adjust your lineup or automatically replace them with the best available bench players:',
                    style: TextStyle(
                  ...
                  ...ineligibleStarters.map((player) {
                    final event = SquadEventService.getPlayerEvent(player.name, _activeSquadEvents);
                    final eventColor = event?.badgeColor ?? AppPalette.coral;
                    final eventIcon = event?.iconData ?? Icons.medical_services_rounded;
                    final title = event?.title ?? 'Unavailable';
                    final timeline = event?.formattedTimeline ?? 'Out for match';
                    final description = event?.description ?? 'Ruled out of current matchday squad.';
                      ...
                      child: Column(
                        ...
                        children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 2),
                                    Text(
                                      '${player.overall} OVR • Age ${player.age?.toInt() ?? 26}',
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                Icon(Icons.timer_outlined, size: 13, color: inkMuted),
                                const SizedBox(width: 6),
                                Expanded(
                                  child: Text(
                                    '$title: $timeline',
            ...
            Row(
              children: [
                TextButton(
                  ...
                  child: Text('CANCEL', style: TextStyle(color: inkMuted, fontSize: 11.5, fontWeight: FontWeight.w700)),
                ),
                const Spacer(),
                OutlinedButton.icon(
                  onPressed: () {
                    Navigator.pop(ctx, false);
                    _jumpToTab(1); // Navigate to SQUAD tab
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        duration: Duration(seconds: 3),
                        backgroundColor: AppPalette.darkSurfaceRaised,
                        content: Text(
                          'Squad Management: Tap any player to make tactical replacements.',
                  ...
                  label: const Text('MANUAL EDIT', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800)),
                ...
                FilledButton.icon(
                  ...
                  label: const Text('AUTO-REPLACE', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800)),
  ...
  Future<void> _simMatchday() async {
    ...
    // Check for any unavailable/ineligible starters before matchday simulation (EA Sports FC Style Squad Selection Alert)
    final ineligibleStarters = SquadEventService.getIneligibleStarters(
      squad: _userSquad,
      activeEvents: _activeSquadEvents,
    );

    if (ineligibleStarters.isNotEmpty) {
      final shouldProceedWithAutoFix = await _showIneligibleStartersDialog(ineligibleStarters);
      if (shouldProceedWithAutoFix != true) {
        return; // Manager chose 'Manual Edit' or 'Cancel'
      }

      // Auto-replace unavailable starters with best available bench options
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor: AppPalette.green.withValues(alpha: 0.9),
            content: const Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Lineup Auto-Fixed: Ineligible starters replaced with best bench options.',
